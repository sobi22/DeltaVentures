...
//   import { setUserRoleDetails } from 'providers/User/userRoleSlice';
...
//   import logo12 from '../../images/logo12.png';
  ...
  const masterMenuItems: MenuItem[] = [
    {
      ...
      icon: <AppstoreOutlined style={smallIconStyle} />,
    },
  ...
  const CategoryMenuItems: MenuItem[] = [
    {
      name: 'Category List',
      path: routes.CATEGORY_MASTER,
      icon: <FileOutlined style={smallIconStyle} />,
    },
    {
      name: 'Add Category',
      path: routes.CATEGORY_MASTER,
      icon: <DollarOutlined style={smallIconStyle} />,
    },
    {
      name: 'Sub Category List',
      path: routes.CATEGORY_MASTER,
  ...
  const menuItems: MenuItem[] = [
    ...
    { name: 'Category', submenus: CategoryMenuItems, icon: <FileOutlined style={mainIconStyle} /> },
    {
      name: 'Cuisines List',
      // path: routes.PROFILE,
      path: routes.CATEGORY_MASTER,
  ...
  const Sidebar = (props: LeftMenuBarProps) => {
    ...
    const [selectedTab, setSelectedTab] = useState<TABS | SUBTABS>('Category List');
    ...
    // const userRoleDetailsQueryResult: any = useGetUserRoleDetailsQuery({});
    // const dispatch = useDispatch();
  
    // useEffect(() => {
    //   if (userRoleDetailsQueryResult.data && userRoleDetailsQueryResult.data.roleAuthenticationDTO) {
    //     dispatch(setUserRoleDetails(userRoleDetailsQueryResult.data.roleAuthenticationDTO.privilegeDTOList));
    //   }
    // }, [userRoleDetailsQueryResult, dispatch]);
    ...
    const hasAccessToSubMenu = (submenu: any): boolean => {
      // const privilegeList = userRoleDetailsQueryResult.data?.roleAuthenticationDTO.privilegeDTOList || [];
      // const matchingPrivilege = privilegeList.find((privilege: any) => privilege.applicationEntityName.toLowerCase() === submenu.name.toLowerCase());
      // return !!matchingPrivilege;
      return true;
    };
    const hasAccessToMenu = (menu: any): boolean => {
      // if (menu.name.toLowerCase() === 'dashboard' || menu.name.toLowerCase() === 'profile') {
      //   return true;
      // } else {
      //   const privilegeList = userRoleDetailsQueryResult.data?.roleAuthenticationDTO?.privilegeDTOList || [];
      //   const matchingPrivilege = privilegeList.some((privilege: any) => privilege.categoryName.toLowerCase() === menu.name.toLowerCase());
      //   return !!matchingPrivilege;
      // }
      return true;
    };
    ...
    return (
      <>
        <Drawer variant="permanent" open={props.drawerOpen} sx={{ width: '100px' }}>
            ...
            <Link to={generatePath(routes.ROOT)}>
              {props.drawerOpen && (
                <Box display={'flex'} alignItems={'center'} gap={2}>
                  <img style={{}} src={''} alt="Small Logo" width={'100%'} />
                </Box>
              )}
            </Link>
          ...
          <Divider />
          {/* <AsyncDOM loading={userRoleDetailsQueryResult.isFetching} error={userRoleDetailsQueryResult.isError}> */}
          {/* <AsyncDOM loading={true} error={false}> */}
            ...
            {/* <AsyncDOM.Content> */}
                ...
                {menuItems.map((item, index) => (
                  <React.Fragment key={item.name}>
                    ...
                    >
                      ...
                      <Typography sx={{ fontSize: '12px', fontWeight: 400,alignItems: 'left' }}>{item.name}</Typography>
              ...
              {/* <Typography sx={{ position: 'fixed', textAlign: 'center', bottom: 0, width: '240px' }}>v{packageJson.version}</Typography> */}
            {/* </AsyncDOM.Content> */}
            {/* <AsyncDOM.Error error={userRoleDetailsQueryResult.error || userRoleDetailsQueryResult.isError}>Unable to show manu items</AsyncDOM.Error> */}
